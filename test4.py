...
import math as math # Importar la llibreria math per a funcions matemàtiques.
...
while True: # Bucle per repetir el programa.
    ...
    while True: # Bucle per assegurar que l'usuari introdueix un grau vàlid.
        try:
            ...
            if grau < 0: # si el grau és més petit que 0, llavors...
            ...
            print(f"Error: {e}. Torna-ho a intentar.") # Mostrar l'error i demanar de nou el grau.
    ...
    def funció_polinòmica(coeficients): # definim una funció amb def i els variables.
        termes= [] #[] és llista buit.
        grau = len(coeficients) -1 # grau del polinomi.
    ...
    #Posem al final -1, perquè el grau del polinomi és de més gran a més petit.
        for i, coef in enumerate(coeficients): # 'enumerate' ens permet recórrer la llista i obtenir a la vegada la posició (índex) i el valor de cada element
            ...
            if exp== 0: # Si exp és 0, el terme serà només el valor del coeficient.
                terme = f"{valor}"
            elif exp == 1:# Si exp és 1, el terme serà el coeficient seguit de "x".
                if valor==1: # Si el valor és 1, només posem "x" sense el coeficient.
                    terme="x"
                else: # Si el valor és diferent de 1, posem el coeficient seguit de "x".
                    terme = f"{valor}x"
            else: # Si exp és més gran que 1, el terme serà el coeficient seguit de "x" elevat a l'exponent.
                if valor==1: # Si el valor és 1, només posem "x" elevat a l'exponent sense el coeficient.
                    terme= f"x^{exp}"
                else: # Si el valor és diferent de 1, posem el coeficient seguit de "x" elevat a l'exponent.
                    terme = f"{valor}x^{exp}"
    ...
    if arrel_exacte is not None: # Si hem trobat una arrel exacta, no cal continuar buscant.
        ...
        tol = 1e-10 # Tolerància per a la convergència del mètode de Newton-Raphson.
        ...
        for i in range(max_iter): # Bucle per a les iteracions del mètode de Newton-Raphson.
            ...
            if abs(x1 - x0) < tol: # Si la diferència entre x1 i x0 és menor que la tolerància, hem trobat una arrel.
                ...
                fx1 = f(x1) # Calcular el valor de la funció en l'arrel aproximada.
                print(f"f({x1:.10f}) = {fx1:.10f}")
                break
            x0 = x1 # Actualitzar x0 per a la següent iteració.

    else: # Si no hem trobat cap interval on hi hagi arrel.
        print("\nNo s'ha trobat cap interval on hi hagi arrel.")

# Mostrar gràfic de la funció 
    
    arrels = [r.real for r in np.roots(coeficients) if np.isreal(r)] #per trobar les arrels reals del polinomi. 
    # np.roots() calcula les arrels del polinomi donat els coeficients.
    # np.isreal() comprova si una arrel és real.

    # triar rang automàticament basat en les arrels reals trobades.
    if arrels: # Si hi ha arrels reals, ajustem el rang al voltant d'elles.
        x_min = min(arrels) - 2 # Afegim un marge de 2 unitats a cada costat.
        x_max = max(arrels) + 2 
    else:
        x_min, x_max = -10, 10 # Si no hi ha arrels reals, utilitzem un rang per defecte.

    x = np.linspace(x_min, x_max, 2000) # 2000 punts entre x_min i x_max per a un gràfic suau.
    y = f(x) # Calcular els valors de la funció en aquests punts.

    plt.figure(figsize=(8,6)) # Mida de la figura 8x6.
    plt.clf() # Netejar la figura actual.
    plt.plot(x, y, label=f"f(x) = {polinomi}", color="blue")# Gràfic de la funció en blau.

# Marcar les arrels reals
    for r in arrels: # Marcar les arrels reals en el gràfic.
        plt.plot(r, 0, 'ro')  # punt vermell a l'arrel.
        plt.text(r, 0, f'{r:.2f}', color='red', fontsize=9, ha='right', va='bottom') # Etiqueta amb el valor de l'arrel.

# Eixos
    plt.axhline(0, color='black', lw=1) # Eix x
    plt.axvline(0, color='black', lw=1) # Eix y

# Quadricula, llegenda, títol i etiquetes
    plt.grid(True, linestyle='--', alpha=0.6) # Quadricula amb línies discontínues i una mica transparents.
